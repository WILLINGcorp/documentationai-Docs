...
<Columns cols={3}>
  ...
  <Card title="v1.4.0" icon="zap" href="#v1-4-0">
    ...
    Events API Developer Portal and dashboard improvements.
  ...
  <Card title="v1.2.0" icon="check-circle" href="#v1-2-0">
    ...
    Video portfolios, webhooks, and bug fixes.
...
<Update label="2026-06-30" description="v1.4.0" tags={["feature", "improvement"]}>

## New Features

- **Events API Developer Portal** — Business, Studio, and Enterprise accounts with MB Events installed can now create OAuth client credentials to build programmatic integrations for event attendees, collaboration proposals, industry profiles, event group chat, and direct messaging. See [MB Events](/mb-events) for the full guide.
- **Developer Section on the Business Dashboard** — A new dashboard section shows whether developer access is unlocked or locked for each available developer program, with a direct link to open the relevant developer portal.

## Improvements

- **Inline redirect URI editing** — Redirect URIs on Events API credential cards can now be added, modified, and removed inline without recreating the credential. Up to 20 URIs per credential, with HTTPS validation (localhost allowed for development).
- **Collapsible dashboard sections** — The Developer Section, capability access control, and sub-account requests on the Business Dashboard can now be collapsed and expanded, with the state persisted across sessions.
- **OAuth scope consent screen updates** — Refreshed the display names and descriptions for six OAuth scopes so they are clearer on the authorization consent screen.

## Bug Fixes

- Fixed developer portal navigation so the "Open developer portal" button routes to the correct Events API developer portal.
...
|---------|--------------|-------------|------|
| v1.4.0 | 2026-06-30 | Events API Developer Portal, dashboard improvements | feature, improvement |
