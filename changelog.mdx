...
<Columns cols={3}>
  ...
  <Card title="v1.4.0" icon="zap" href="#v1-4-0">
    ...
    MCP integration, Events developer portal, OAuth improvements.
...
<Update label="2026-07-04" description="v1.4.0" tags={["feature", "improvement", "bugfix"]}>

## New Features

- **MCP (Model Context Protocol) integration** — AI agents can now connect to your ModelBoard account via OAuth and access four read-only tools: view your profile, search creators, search businesses, and list your installed apps. See [Integrations](/integrations).
- **MB Events Developer Portal** — Business and studio accounts with MB Events installed can now create and manage OAuth clients, select API scopes, and access quickstart code snippets for the Events API. See [MB App Store](/mb-apps).
- **Inline redirect URI management** — Add, edit, and remove redirect URIs directly on each OAuth client card in the developer portal.

## Updates

- Redesigned OAuth consent screen with categorized permission cards (basic, sensitive, write) and a visual connection flow diagram.
- Refined scope display names in the consent and scope picker for clearer permission descriptions.
- Dashboard sections — Developer, Capability Matrix, and Sub-Account Requests — now remember their expanded or collapsed state between sessions.
- Login now redirects you back to your original page after signing in when following a link from another part of the platform.

## Bug Fixes

- Fixed OAuth refresh tokens silently ignoring narrowed scope requests — scopes are now properly validated and applied on token refresh.
- Fixed the "Open developer portal" button for MB Events navigating to the wrong page.
...
<Expandable title="Preview: v1.5.0 (ETA Q3 2026)" default-open="true">
...
|---------|--------------|-------------|------|
| v1.4.0 | 2026-07-04 | MCP integration, Events developer portal, OAuth improvements | feature, improvement, bugfix |
