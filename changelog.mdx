...
<Columns cols={4}>
  ...
  <Card title="2026-06-10" icon="shield" href="#security-and-organization-updates">

    Security, privacy, and organization access updates.
...
<Update label="2026-06-10" description="Security and organization updates" tags={["security", "improvement"]}>

## Security and privacy

- Added admin guidance for Security Monitoring, including alert triage, threshold changes, authentication rate-limit behavior, and audit history.
- Documented OAuth Allowed Origins so admins can control trusted redirect origins, manage preview URL patterns, and review rejected-origin audit events.
- Clarified that MB Events session attendance should show individual records only to attendees and organizers, while public views use aggregate headcounts.
- Added Help Center disclosure for diagnostic error monitoring and limited session replay, including the context support teams may use when troubleshooting.
- Clarified portfolio album visibility after public album access was tightened, so creators can check whether albums are intentionally public, link-only, or restricted.

## Organization access

- Added the Capability Matrix model for organization staff access, including default staff capabilities, owner-only controls, save, discard, and reset behavior.
- Unified organization access management around Sub-Account Requests, with separate Invites, Join, Incoming, and Outgoing tabs for invitations, joins, and transfers.
- Updated business listing guidance to explain that owners manage staff access through the organization workflow instead of public Business Page fields.
...
|---------|--------------|-------------|------|
| Security and organization updates | 2026-06-10 | Capability Matrix, unified invitations, admin security, privacy clarifications | security, improvement |
