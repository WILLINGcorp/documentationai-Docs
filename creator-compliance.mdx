...
- Completing age or 2257‑related checks when they apply to your work
- Entering your date of birth and any requested document expiration date using the verification dropdown fields and confirming they match your ID
- Capturing the front of your ID, and the back as well when your document type requires it, by choosing either **Take Photo** or **Upload**, keeping the document upright in portrait orientation and following the crop frame so every edge stays visible
- Reviewing the consent flow for automated 2257 signature delegation when ModelBoard offers it for your account
- Managing automatic 2257 signing from settings, where you can enable it for future eligible paperwork or revoke it at any time
- Checking the vault or compliance audit view to confirm whether delegation is pending, active, used on a document, or revoked
...
- A personal compliance card showing your 2257 documentation status, delegated signing status when it applies, boundaries and consent status, and QR scan activity
...
- Enter your legal details exactly as they appear on your documents, including your date of birth and any requested document expiration date using the dropdown fields
- Choose the correct ID type before you continue
- Capture the front of your ID with **Take Photo** or **Upload**, keeping the document upright in portrait orientation and aligned to the crop frame
- Capture the back of your ID as well when the document type requires it, using the same portrait framing so every edge stays visible
- Confirm that each image is clear, readable, and correctly categorized before you continue
- Read the automated 2257 signature delegation consent flow carefully if it appears during verification
- Choose whether to allow automatic signing for eligible 2257 paperwork, knowing you can change that choice later from settings
- Submit any required forms and wait for the status to update

**Success looks like**

You see a clear verification status such as `in review` or `approved`, along with any delegation status that applies to future 2257 signing. Your creator profile shows an updated verificationStatus indicator, and your settings reflect whether automatic signing is currently enabled.
...
If the verification flow does not let you continue, check that every required ID image is attached, that your document fits fully inside the crop frame, and that your date fields were selected from the dropdowns rather than left blank.
...
- Review the vault or compliance audit view when you need to confirm when automated 2257 signing was authorized, used, or revoked
- Periodically review which documents may be expiring soon and update them before they lapse

**Success looks like**

Your vAulth‑connected checks show a completed or current status in your Compliance area or project compliance card, and the audit view gives you a clear history of delegation activity without exposing the underlying documents.
...
- If your account is eligible, you may also see a consent flow that lets ModelBoard automatically sign future 2257 paperwork on your behalf
- You can later enable or revoke that automatic signing choice from settings
- The vault or compliance area shows an audit view so you can confirm when delegation was granted, used, or revoked
- Your actual documents stay encrypted and are not visible to other collaborators in ModelBoard
- The project only sees a status such as `pending`, `current`, or waiting on manual action on your compliance card
...
- If the project expects delegated signing, confirm in settings whether automatic signing is still enabled for your account
- Wait for the project2257Status indicator to update from pending to a more complete state

If the status does not update after you complete the vAulth flow, confirm with the organizer that they see the same status, review the delegation audit view for recent activity, and then reach out to support for help reconciling the view.
