...
- Reviewing any prompt to authorize an Automated 2257 Signature after verification
- Tracking which items are still pending, complete, or ready for your next action
...
The Compliance tab in a project integrates with vAulth to verify documentation, signatures, and 2257 readiness. Sensitive documents stay encrypted and do not display directly in the UI.
...
- Capture your ID in portrait orientation when the mobile verification flow asks for a document photo
- Enter the ID expiration date in the format the form requests; ISO-style dates such as `YYYY-MM-DD` are accepted when shown by your document or system keyboard
- Complete the vAulth mobile flow if the desktop screen sends you to your phone
- Submit any required forms and wait for the status to update

**Success looks like**

You see a clear verification status such as `in review`, `verified`, or `approved`, with a list of remaining items if anything is still pending. Your creator profile shows an updated verificationStatus indicator.

<Callout kind="info">

If you begin verification on desktop and continue on mobile, keep the desktop window open. The desktop experience waits for the full mobile verification and signature authorization flow to finish before it shows the Thank You or completion state.
...
Some checks, such as 2257 documentation, rely on vAulth to store, verify, and authorize sensitive records.
...
- Review the Automated 2257 Signature prompt if it appears after verification completes
- Choose whether to authorize or decline the automated signature authorization; either choice allows the verification completion flow to continue
- Manage or revoke the authorization later from your settings if your consent changes
- Periodically review which documents may be expiring soon and update them before they lapse

**Success looks like**

Your vAulth‑connected checks show a completed or current status in your Compliance area or project compliance card, and automated signature activity appears in your vAulth dashboard when it applies to your account.

<Callout kind="tip">

Revoking Automated 2257 Signature authorization does not erase completed verification. Future documents may require manual signing after revocation, so review new project prompts carefully.
...
### Automated 2257 Signature authorization

Shows whether you authorized Automated 2257 Signature use after verification. Authorizing or declining this prompt does not block verification completion, and you can manage or revoke authorization later from settings.

### Automated signature activity

Summarizes automated signature activity when it applies to your account. Review the vAulth dashboard for activity history and use settings if you need to change your authorization.
...
For work that involves 2257‑relevant content, projects rely on vAulth to handle document storage, verification, signature authorization, and audit‑readiness.
...
- Verification completion may ask whether you want to authorize an Automated 2257 Signature for eligible future documents
- Authorizing or declining the prompt still lets the verification completion flow continue
...
- Automated signature activity appears in the vAulth dashboard when it applies to your account

Use these flows instead of sending ID photos or documents through messages or external channels. If you revoke authorization later, expect some future 2257 documents to ask for a manual signature before the project can treat them as complete.
...
- **Review automated signature prompts before you authorize them.** Automated 2257 Signature authorization can reduce repeat signing friction, but you should understand what you are approving and revoke it from settings if your consent changes.
...
- Your ID capture followed the mobile flow's portrait-first guidance
- Your profile name and birthdate line up with what appears on your documents
- Your expiration date uses an accepted format, including ISO-style dates such as `YYYY-MM-DD` when the field supports them
...
- Review any Automated 2257 Signature prompt that appears after verification completes
...
<Expandable title="What happens if I decline or revoke Automated 2257 Signature authorization?" default-open="false">

Declining the authorization prompt does not stop verification from completing. It means eligible future 2257 documents may need your manual signature instead of using automated signature authorization.

If you authorize the prompt and later change your mind, manage or revoke the authorization from settings. After revocation, review new project compliance prompts carefully because future documents may ask you to sign manually before they can be treated as complete.

When automated signatures apply, activity is surfaced in your vAulth dashboard so you can review what happened without exposing sensitive documents in the main project UI.
