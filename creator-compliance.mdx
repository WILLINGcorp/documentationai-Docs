...
description: >-
  Understand how creator compliance works in ModelBoard, how to stay verified,
  and how to reduce friction when joining collaborations and projects.
---
...
- When vAulth asks you to capture an ID on mobile, keep your phone in portrait orientation and fit the document into the landscape guide frame shown on screen; you do not need to rotate the phone manually
...
- After drawing a signature, review any optional authorization to save and reuse that signature solely for future automated 2257 document generation
- If your browser asks for location during authorization, choose whether to share it; declining location access should not stop normal completion of the current verification or signing flow
- Periodically review which documents may be expiring soon and update them before they lapse

**Success looks like**

Your vAulth‑connected checks show a completed or current status in your Compliance area or project compliance card, and you do not see repeated prompts for the same documentation. If you authorize automated signature reuse, future eligible 2257 document generation can use that saved signature, and each automated use appears in your Vault audit history.
...
- Decide whether to authorize vAulth or ModelBoard to save and reuse that signature solely for future automated 2257 document generation

As more collaborators sign, the project sees a signature progress indicator move toward completion. The final signed PDF becomes available only after all required signatures are complete. Optional automated signature authorization is separate from signing the current release: you can decline it and still complete the normal signing flow.
...
- Mobile ID capture is portrait‑first: hold your phone upright and align the document with the landscape guide frame instead of rotating the phone manually
- After you draw a signature, you may be asked whether vAulth or ModelBoard can save and reuse that signature solely for future automated 2257 document generation
- Authorization may request browser geolocation to record context for the authorization; you can decline geolocation and continue the normal completion flow
...
Use these flows instead of sending ID photos or documents through messages or external channels. You can revoke automated signature authorization later from settings, and automated 2257 signature use appears in your Vault audit history.

### Capture IDs on mobile with vAulth

vAulth mobile ID capture lets you photograph required identity documents from your phone as part of a compliance or 2257 verification flow.

Before you start, make sure your ID is unexpired, your camera lens is clean, and you are in a well-lit place. When the capture screen opens, keep the phone upright in portrait orientation and place the ID inside the landscape guide frame; do not rotate the phone manually. After you submit, return to the ModelBoard or vAulth flow and wait for the project or dashboard status to update.

<Callout kind="info">

Mobile ID capture is different from a project QR scan. ID capture collects document photos for verification. A project QR scan records an in-person verification event tied to a Collab project.


</Callout>

### Understand automated 2257 signature delegation

Automated 2257 signature delegation applies to eligible 2257 document generation, not to the project release form you are reviewing and signing in the Compliance tab.

When the option appears, you decide whether vAulth or ModelBoard may save and reuse your drawn signature solely for future automated 2257 document generation. If you decline, you can still complete the current verification or signing flow and provide signatures manually when required. If you authorize reuse, automated uses appear in Vault audit history, and you can revoke the authorization later from settings.
...
- **Review signature authorization separately from signing.** Saving a signature for automated 2257 document generation is optional, limited to that purpose, and can be declined without blocking normal completion.
- **Revoke authorization when your preference changes.** If you no longer want automated 2257 generation to use your saved signature, revoke that authorization from settings and review your Vault audit history for prior automated uses.
...
- Automated 2257 signature use is tracked in Vault audit history rather than exposing your stored signature to collaborators
...
<Expandable title="Do I have to authorize saved signature reuse for automated 2257 documents?" default-open="false">

No. After drawing a signature, you may be offered the option to let vAulth or ModelBoard save and reuse that signature solely for future automated 2257 document generation.

Declining that authorization should not block the current verification or signing flow. If you authorize reuse, your browser may ask for geolocation as part of the authorization context, and each automated use appears in your Vault audit history. You can revoke the authorization later from settings.
