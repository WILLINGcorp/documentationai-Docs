...
The File Sharing tab is the central place for storing, organizing, and sharing project assets across two stages: Post-Production working files and Distribution delivery-ready files.

<Callout kind="info">

File Sharing serves two stages: Post-Production project files and Distribution delivery. Keep both organized as the project progresses.

</Callout>

## When to update it

- **When files are uploaded or removed**: Keep the file list current as assets are added or deleted.
- **When a file status changes**: Update metadata when files move from draft to approved to delivery-ready.
- **When the project changes stages**: Confirm post-production files are finalized before moving to distribution.
- **When new collaborators join**: Ensure they have access to the files they need for their role.
- **When feedback requires revisions**: Upload new versions and update the status of affected files.
- **Before delivery**: Verify all delivery-ready files are present, approved, and correctly labeled.

## What to include

- **Approved deliverables**: Files that have been reviewed and approved for sharing or delivery.
- **Working files**: Drafts, in-progress edits, or source materials that collaborators need during post-production.
- **Version history**: Previous versions of files kept for reference or rollback.
- **File metadata**: Title, description, status, version, and assigned collaborator for each file.
- **Access permissions**: Which collaborators can view, download, or manage each file.

## Best practices

- **Name files consistently**: Use a naming convention that includes version, date, and status so collaborators can identify the correct file at a glance.
- **Set access at upload**: Determine who can view, download, or manage a file when uploading it, not after.
- **Keep delivery files separate from working files**: Use workflow groups or filters to separate approved deliverables from in-progress drafts.
- **Update status immediately**: When a file is approved or rejected, update its review status so the team always knows the current state.
- **Verify before distribution**: No file should move to distribution delivery without explicit approval and correct metadata.

## Role-specific guidance

<Tabs>
<Tab icon="user" title="Project owner (sender)">

You are responsible for approving files for delivery, setting access permissions, and ensuring the file list stays organized. Confirm all delivery-ready files are approved before moving to the Distribution stage. Use review statuses to signal which files are cleared for downstream use.

</Tab>

<Tab icon="users" title="Collaborator (receiver)">

Your role is to upload working files, check file statuses, and download the files you need. If a file you need is missing or access-restricted, request it from the project owner in project chat. Use tags and comments to keep feedback attached to the right asset.
