...
Your dashboard may show setup and recovery messages shortly after you sign in, especially if your account is new or you left onboarding before finishing every step. These messages are designed to keep you moving without sending you back through setup unnecessarily.

<Callout kind="info">

If you recently created your account, ModelBoard may keep you on the dashboard while profile or session details finish loading. Wait for the dashboard to settle before deciding that onboarding did not save.

</Callout>

### Resume an incomplete setup

If you left onboarding partway through, your dashboard may show a **Resume Setup** banner. Use the banner to continue from the remaining setup items instead of starting over.

Choose **Resume Setup** when you are ready to finish profile, verification, or account details. If you need to handle an urgent invite first, you can continue using the dashboard and return to the banner later.

### Reload a failed dashboard widget

Dashboard sections can load independently. If one widget cannot load while the rest of the dashboard is available, that widget may show a recovery message with a reload option.

Select the reload action on the affected widget first. If the widget still does not recover, refresh the page and check [Page not loading](/help-center/troubleshooting/page-not-loading) for broader troubleshooting steps.

### Respond to verification reminders

If you skipped verification or your identity status is not complete, an identity verification prompt may appear shortly after the dashboard opens. The prompt may offer choices such as starting verification now, continuing later, or reviewing what is required.

Choose the verification option when you have your documents ready. If you dismiss or delay the prompt, expect verification reminders to return until your status is complete.
