...
Start or review identity checks, consent records, automated 2257 signature delegation, and other compliance-by-design workflows tied to your collaborations and profile.
...
Fine-tune privacy, notification preferences, connected apps, and the signature-delegation settings card for automatic 2257 signing on eligible paperwork.
...
- Enter your date of birth using the built-in date picker so it matches the ID you plan to submit.
- Choose your ID type, then add the front image with **Take Photo** or **Upload**.
- Add the back image too when your document type requires it.
- Read the automated 2257 signature delegation consent flow carefully if the platform presents it during verification.
- Decide whether to keep automatic signing enabled for eligible 2257 paperwork, and note that you can change that choice later from settings.
...
**Success looks like:** Your profile shows that verification is in progress or complete, your compliance area does not list urgent missing essentials, and you know whether automatic signing is currently enabled for your account.
...
- Review the signature-delegation settings card if you want to enable or revoke automatic 2257 signing.
- If you need to confirm recent delegation activity, return to the vault or compliance area to review the audit history for that setting.
- Decide which notification channels you rely on for time-sensitive collab updates.

**Success looks like:** Your notifications are not cluttered with irrelevant alerts, and you know exactly how you will be notified about invitations, schedule changes, compliance requests, changes to your automatic signing preference, and where to confirm delegation activity later.
...
- Review your settings if you need to confirm whether automated 2257 signing is still enabled before the next project asks for paperwork.
...
- If the repeated prompt is tied to 2257 paperwork, confirm whether automatic signing is enabled in settings or whether the project now needs a manual step instead.
