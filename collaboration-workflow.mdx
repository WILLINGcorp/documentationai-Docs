...
<Steps>
  ...
  <Step title="Mark the project completed and archive when ready" icon="upload" title-type="p">
    ...
    - Archive the project when you no longer need it in your active list but still want to keep a record.

    **Success looks like** the project shows as `completed` or `archived` in `/collabs`, and you can still open `/collab/{id}` to review history and assets.
  </Step>
</Steps>
...
<Callout kind="info" collapsed="false">
  Different tabs may be more or less important depending on your role. Owners typically focus on Collaborators, Compliance, and Release Assets, while collaborators spend more time in Files, Post-Editor, and project chat.
</Callout>
...
<ExpandableGroup>
  <Expandable title="An invite stays pending and the collaborator has not joined" default-open="true">
